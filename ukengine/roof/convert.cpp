...
--------------------------------------------------------------------------------
Purpose:
This file is part of the Vietnamese Encoding Converter Library (VnConv) within
the UniKey project. It provides functionality for converting text between
different Vietnamese character sets. The file includes functions for stream,
buffer, and file-based conversions, as well as character set transformations
such as case conversion and tone removal.

Key Functions:
1. vnFileStreamConvert: Converts a file stream from one Vietnamese charset to
	another.
2. genConvert: A general-purpose function for converting Vietnamese character
	sets using input and output streams.
3. VnConvert: Converts Vietnamese text between two character sets, managing
	memory for input and output buffers.
4. VnFileConvert: Handles file-based conversions, including support for
	standard input/output and temporary files.
5. VnConvErrMsg: Retrieves human-readable error messages for conversion errors.

Usage Context:
This file operates as a backend utility for encoding conversions. Its
functionality might be indirectly accessible through:
- Input Method Settings: If the UniKey engine is integrated into an input
  method framework like IBus, users might configure encoding options in the
  settings.
- Command-Line Tools: If the project includes command-line utilities, this
  file's functionality could be invoked for batch conversions or testing.
--------------------------------------------------------------------------------*/
